...
def main():
    ...
    text_tokenizer.fit_on_texts(x_data_processed)
    tensor_processed = text_tokenizer.texts_to_sequences(x_data_processed)
    tensor_processed = tf.keras.preprocessing.sequence.pad_sequences(tensor_processed, padding='post')
    ...
    # Modifying targets to be in range [0..1]
    ...
    # Create the model
    rnn = model.Model(input_dim=vocab_size, dropout=0.5, epochs=3, batch_size=128, validation_split=0.2)
